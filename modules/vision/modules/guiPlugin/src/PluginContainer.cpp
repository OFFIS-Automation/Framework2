...
void PluginContainer::loadPlugins(QString folder)
{
    ...
    foreach(QString file, files)
    {
        ...
        if(obj)
        {
            VisionGuiPluginInterface* plugin = qobject_cast<VisionGuiPluginInterface*>(obj);
            if(plugin)
               addPlugin(plugin);
        }
    }
...
void PluginContainer::addPlugin(VisionGuiPluginInterface *plugin)
{
...
{
    QListIterator<VisionGuiPluginInterface*> plugins(mPlugins);
    while(plugins.hasNext())
    {
        VisionGuiPluginInterface* plugin = plugins.next();
...
OverlayInterface *PluginContainer::overlayFor(const QString &name, bool output, bool isMasterOverlay, VisionInterface *visionInterface)
{
    foreach(VisionGuiPluginInterface* plugin, mPlugins)
    {
...
OverlayInterface *PluginContainer::overlayFor(const PortInfo &info, bool isOutput, bool isMasterOverlay, VisionInterface *visionInterface)
{
    foreach(VisionGuiPluginInterface* plugin, mPlugins)
    {
